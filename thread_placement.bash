    ...
    for P in "${PLACEMENTS[@]}"; do
        for T in "${THREADS[@]}"; do
            ...
            for VERSION in 1 2; do  # Run both parallel versions
                ...
                output=$(./poisson_j_thread $N 1000 1e-6 20 $VERSION)

                # Debugging: Print full output
                echo "Output for Grid Size: $N, Threads: $T, Placement: $P, Version: $VERSION"
                echo "$output"

                # Extract the **last** numerical value (execution time)
                execution_time=$(echo "$output" | awk 'NF>0 {last=$NF} END {print last}')
